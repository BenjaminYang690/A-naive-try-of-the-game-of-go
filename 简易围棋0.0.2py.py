...
class gogame:
    ...
    def set_playername(self,blackname='Black',whitename='White'):
        self.info['PB']=blackname
        self.info['PW']=whitename
    ...
    def init_draw(self):# 画出棋盘
        ...
        r=33*2/5
        a=(self.SZ-1)//2
        tt.screensize(self.SZ*40,self.SZ*40,'burlywood')
    ...
    def show_latest(self):# 在图像上显示最近一步着手
        ...
        if (len(self.record)==1 and self.record[0][-2:]=='[]') or len(self.record)==0:
            return
    ...
    def strategy(self):# 推荐着手
        ...
        def same_form(dt):
            ...
            while self.record[idx][0]=='d' or self.record[idx][-2:]=='[]':
                idx-=1
            past=self.record[idx]
            self.last()
            pastelems=self.elems
            self.nxt(past)
            self.nxt(dt)
            tobeelems=self.elems
            self.last()
            if pastelems==tobeelems:
                return True
            return False
        ...
        if latest[0]=='W':
            ...
            for i in range(self.SZ):
                for j in range(self.SZ):
                    ...
                    valuelst.append([i,j,vl])
                    self.last()
            sortlst=merge_sort(valuelst,2)
            if sortlst[-1][-1]<-self.SZ**2//4:
                return 'B[]'
            maxlst=[]
            for c in sortlst:
                if c[2]==sortlst[-1][2]:
            ...
            if same_form(Nxtdt):
                sortlst.remove([x,y,sortlst[-1][2]])
                maxlst=[]
                for c in sortlst:
                    if c[2]==sortlst[-1][2]:
                        maxlst.append(c[:2])
...
class Player:
    ...
    def __init__(self,game,color='B',Type='P'):
        self.game=game
        self.color=color
        self.Type=Type
        playername=input('Input your name(your color is {}): '.format(color))
    ...
    def get_nxt(self):
        ...
        def input_pos():
            ...
            while True:
                i,j=input('{}\'s turn. Please input the position. \'0 0\' to retract. \'-1 -1\' to exit.'.format(self.playername)).split()
                if i=='-1' and j=='-1':
                    break
                if i=='0' and j=='0':
                    break
...
def demo():
    ...
    if Mode==0:
        ...
        ply1=Player(g,'B','P')
        ply2=Player(g,'W','P')
        g.set_playername(ply1.playername,ply2.playername)
    elif Mode==1:
